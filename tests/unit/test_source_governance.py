...
async def test_admit_refuses_and_trips_the_breaker_one_claim_over_the_ceiling() -> None:
    ...
    service = _service(router)
    before = _sample("registry_source_ingest_breach_total", tenant_id=str(tenant_id), source_id=str(source_id))
    ...
    after = _sample("registry_source_ingest_breach_total", tenant_id=str(tenant_id), source_id=str(source_id))
...
async def test_admit_increments_the_admitted_counter_by_the_admitted_count() -> None:
    ...
    service = _service(router)
    before = _sample("registry_source_ingest_admitted_total", tenant_id=str(tenant_id), source_id=str(source_id))

    await service.admit(source_id, count=7)

    after = _sample("registry_source_ingest_admitted_total", tenant_id=str(tenant_id), source_id=str(source_id))
