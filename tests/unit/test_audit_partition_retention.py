...
from contextplane.wiring.jobs import (
...
class TestCheckAuditPartitionAges:
    ...
    async def test_gauge_set_to_count_of_eligible(self) -> None:
        ...
        with patch(
            "contextplane.wiring.jobs.audit_partitions_eligible_for_archival",
    ...
    async def test_warning_emitted_when_eligible(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="contextplane.wiring.jobs"):
            with patch(
                "contextplane.wiring.jobs.audit_partitions_eligible_for_archival",
        ...
        assert "docs/06-operations/01-ops.md" in caplog.text
