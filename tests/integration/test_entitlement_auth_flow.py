...
#: The deployment's middle token in the entitlement grammar
#: ``<tenant_slug>_<DISCRIMINATOR>_<ROLE>``. Named once and used to build every
#: entitlement string below, because the parser splits on it: a settings value
#: and a hand-written string that disagree drop every grant, and the request
#: then fails as a 403 that looks like an authorization decision rather than a
#: fixture that no longer matches its own deployment.
_DISCRIMINATOR = "CONTEXTPLANE"


def _entitlement(tenant_slug: str, role: str) -> str:
    return f"{tenant_slug}_{_DISCRIMINATOR}_{role}"
...
def _settings(pg_url: str) -> Settings:
    return Settings(
        ...
        entitlement_service_discriminator=_DISCRIMINATOR,
...
async def test_success_one_tenant_returns_200(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    ...
    fetcher.return_value = [_entitlement("t-success-1", "ADMIN")]
...
@pytest.mark.asyncio
async def test_empty_entitlements_returns_403(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    app, fetcher = app_with_resolver
...
async def test_disabled_tenant_returns_403_and_does_not_modify_row(
    ...
    try:
        ...
        fetcher.return_value = [_entitlement(slug, "ADMIN")]

        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            with _patch_validator_returning({"sub": "u-disabled", "iat": 1, "exp": 9999999999}, "u-disabled"):
                resp = await client.get("/v1/whoami", headers=_bearer_headers())
...
@pytest.mark.asyncio
async def test_auth_rejected_401_propagates(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    app, fetcher = app_with_resolver
...
@pytest.mark.asyncio
async def test_5xx_cold_cache_returns_503(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    app, fetcher = app_with_resolver
...
@pytest.mark.asyncio
async def test_success_multi_tenant_requires_x_tenant_id_header(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    ...
    app, fetcher = app_with_resolver
    fetcher.return_value = [
        _entitlement("t-multi-a", "ADMIN"),
        _entitlement("t-multi-b", "CONSUMER"),
    ]

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        with _patch_validator_returning({"sub": "u-multi", "iat": 1, "exp": 9999999999}, "u-multi"):
            # No X-Tenant-ID → 400.
        ...
        with _patch_validator_returning({"sub": "u-multi", "iat": 1, "exp": 9999999999}, "u-multi"):
            resp_with_header = await client.get("/v1/whoami", headers=_bearer_headers(**{"X-Tenant-ID": "t-multi-b"}))
        assert resp_with_header.status_code == 200
        assert resp_with_header.json().get("tenant_slug") == "t-multi-b"


@pytest.mark.asyncio
async def test_unknown_role_drops_to_403(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    """All entitlement entries have role suffixes outside the mapping
    → parser drops them all → resolver returns empty grants → 403."""
    app, fetcher = app_with_resolver
    fetcher.return_value = [_entitlement("t-ghost", "GHOST_ROLE")]

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        with _patch_validator_returning({"sub": "u-ghost", "iat": 1, "exp": 9999999999}, "u-ghost"):
            resp = await client.get("/v1/whoami", headers=_bearer_headers())
...
@pytest.mark.asyncio
async def test_malformed_upstream_returns_503(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    """Upstream entitlement service returns a malformed body →
    EntitlementMalformedError → 503 (no cache fallback consulted)."""
    app, fetcher = app_with_resolver
    fetcher.side_effect = entitlement_client.EntitlementMalformedError("non-JSON body")

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        with _patch_validator_returning({"sub": "u-malformed", "iat": 1, "exp": 9999999999}, "u-malformed"):
            resp = await client.get("/v1/whoami", headers=_bearer_headers())
...
@pytest.mark.asyncio
async def test_timeout_cold_cache_returns_503(app_with_resolver: tuple[FastAPI, AsyncMock]) -> None:
    """Upstream entitlement service times out with no warm cache →
    EntitlementServiceError (cacheable=True but cold cache) → 503."""
    app, fetcher = app_with_resolver
    fetcher.side_effect = entitlement_client.EntitlementServiceError("transport error: ReadTimeout")

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        with _patch_validator_returning({"sub": "u-timeout", "iat": 1, "exp": 9999999999}, "u-timeout"):
            resp = await client.get("/v1/whoami", headers=_bearer_headers())
