...
async def test_no_usage_response_model_can_carry_an_actor_id(harness: _Harness) -> None:
    ...
    usage_ops = [
        op for path, item in schema["paths"].items() if path.startswith("/v1/admin/usage") for op in item.values()
    ]
...
async def test_a_backwards_window_is_a_422_not_a_500(harness: _Harness) -> None:
    ...
    resp = await _get(app_harness, client, persona, "/v1/admin/usage/summary?from=2026-05-20&to=2026-05-01")
