...
from contextplane.extraction.response_factory import JudgeFamilyRefused, PanelMember
...
def _member(position: int, selector: str, provider: _JudgeProvider) -> PanelMember:
    return PanelMember(model_pin="", position=position, provider=provider, selector=selector)


def _panel_service(world: dict[str, Any], panel: tuple[PanelMember, ...]) -> JudgementService:
    return JudgementService(
        candidate_model_pin="",
        candidate_selector="anthropic",
        clock=FakeClock(_NOW),
        model_pin="",
        panel=panel,
        provider=None,
        provider_selector="noop",
        session_factory=world["factory"],
        simulations=_Simulations(world["simulation"]),  # type: ignore[arg-type]
    )
...
@pytest.mark.asyncio
async def test_a_two_to_one_panel_is_recorded_as_two_to_one(world: dict[str, Any]) -> None:
    """Averaging would destroy the signal the panel costs 3x to produce."""
    agree = {CRITERION_GROUNDEDNESS: "pass", CRITERION_RELEVANCE: "pass"}
    dissent = {CRITERION_GROUNDEDNESS: "fail", CRITERION_RELEVANCE: "pass"}
    panel = (
        _member(0, "openai", _JudgeProvider(verdicts=agree)),
        _member(1, "mistral", _JudgeProvider(verdicts=agree)),
        _member(2, "cohere", _JudgeProvider(verdicts=dissent)),
    )

    outcomes = await _panel_service(world, panel).judge_panel(world["ctx"], simulation_id=world["simulation_id"])

    grounded = next(entry for entry in outcomes if entry.criterion == CRITERION_GROUNDEDNESS)
    assert grounded.votes == {"pass": 2, "fail": 1}
    assert grounded.majority == "pass"
    assert grounded.is_split is True

    relevance = next(entry for entry in outcomes if entry.criterion == CRITERION_RELEVANCE)
    assert relevance.is_split is False
    assert relevance.majority == "pass"


@pytest.mark.asyncio
async def test_an_evenly_split_panel_reports_no_majority_rather_than_a_tie_break(world: dict[str, Any]) -> None:
    """A panel that split evenly has not decided; inventing a winner would report agreement nobody reached."""
    panel = (
        _member(0, "openai", _JudgeProvider(verdicts=dict.fromkeys(JUDGED_CRITERIA, "pass"))),
        _member(1, "mistral", _JudgeProvider(verdicts=dict.fromkeys(JUDGED_CRITERIA, "fail"))),
    )

    outcomes = await _panel_service(world, panel).judge_panel(world["ctx"], simulation_id=world["simulation_id"])

    assert all(entry.majority is None for entry in outcomes)
    assert all(entry.is_split for entry in outcomes)


@pytest.mark.asyncio
async def test_a_panel_whose_members_share_a_family_is_refused_and_names_both(world: dict[str, Any]) -> None:
    """Three judges from one family cancel nothing."""
    panel = (
        _member(0, "openai", _JudgeProvider()),
        _member(1, "openai", _JudgeProvider()),
    )
    with pytest.raises(JudgeFamilyRefused, match="positions 0 and 1"):
        await _panel_service(world, panel).judge_panel(world["ctx"], simulation_id=world["simulation_id"])


@pytest.mark.asyncio
async def test_a_panel_member_sharing_the_candidates_family_is_refused(world: dict[str, Any]) -> None:
    panel = (
        _member(0, "openai", _JudgeProvider()),
        _member(1, "anthropic", _JudgeProvider()),
    )
    with pytest.raises(JudgeFamilyRefused, match="both 'anthropic'"):
        await _panel_service(world, panel).judge_panel(world["ctx"], simulation_id=world["simulation_id"])


@pytest.mark.asyncio
async def test_no_panel_configured_says_which_settings_to_set(world: dict[str, Any]) -> None:
    service = _panel_service(world, ())
    assert service.panel_size == 0
    with pytest.raises(SimulationUnavailable, match="JUDGE_2_PROVIDER"):
        await service.judge_panel(world["ctx"], simulation_id=world["simulation_id"])


@pytest.mark.asyncio
async def test_each_panel_member_occupies_its_own_position(world: dict[str, Any]) -> None:
    """Two members are two rows; a re-run at one position is not a second opinion."""
    panel = (
        _member(0, "openai", _JudgeProvider()),
        _member(1, "mistral", _JudgeProvider()),
    )
    service = _panel_service(world, panel)
    await service.judge_panel(world["ctx"], simulation_id=world["simulation_id"])
    await service.judge_panel(world["ctx"], simulation_id=world["simulation_id"])

    read_back = await service.judgements_of(world["ctx"], world["simulation_id"])
    grounded = [entry for entry in read_back if entry.criterion == CRITERION_GROUNDEDNESS]
    assert sorted(entry.panel_position for entry in grounded) == [0, 1]


@pytest.mark.asyncio
async def test_an_unfitted_judge_is_reported_as_uncalibrated(world: dict[str, Any]) -> None:
    """Until a fit exists for the tuple, the verdict is unproven and the surface says so."""
    service = _service(world, provider=_JudgeProvider())
    await service.judge(world["ctx"], simulation_id=world["simulation_id"])
    assert await service.calibrated_tuples() == frozenset()
