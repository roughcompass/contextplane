...
@pytest.mark.asyncio
async def test_calls_from_several_tenants_are_summed_without_naming_them(
    ...
    try:
        ...
        body = resp.text
        assert str(tenant_a) not in body and str(tenant_b) not in body, (
...
@pytest.mark.asyncio
async def test_a_capability_owned_by_someone_else_is_not_in_the_answer(
    pg_container: str, harness: _Harness
) -> None:
...
async def test_a_window_wider_than_the_cap_is_a_422(harness: _Harness) -> None:
    ...
    resp = await _get(
        app_harness, client, owner, "/v1/usage/owned-capabilities?from=2020-01-01&to=2026-01-01"
    )
