...
async def test_rest_and_mcp_are_summarised_separately(factory) -> None:
    ...
    summary = await reads.read_summary(factory, tenant_id=tenant, start=_DAY, end=_DAY, retention_days=90, today=_TODAY)
...
async def test_another_tenants_usage_is_not_in_the_answer(factory) -> None:
    ...
    summary = await reads.read_summary(factory, tenant_id=mine, start=_DAY, end=_DAY, retention_days=90, today=_TODAY)
