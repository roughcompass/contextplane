"""ARC write-path conformance gate: no mutation outside the service layer.

Every ARC write goes through `registry/arc/service/`. That is not a
stylistic preference — it is the only reason the guarantees elsewhere in
this subsystem hold. Receipts are atomic with their audit rows because one
service writes both in one transaction; challenges are single-use because
one service holds the lock; content is encrypted because one service owns
the envelope. A router or worker that issued its own `UPDATE` would bypass
all of that while still looking like ordinary code.
...
may appear only under `registry/arc/service/`. Anywhere else in the package
— models, schemas, types — it is a failure.

Two tests, and the second matters as much as the first: a negative fixture
proves the walker actually detects what it claims to, so this cannot pass
by silently finding nothing.
"""
...
# The one subtree permitted to mutate.
...
def test_no_sql_mutation_outside_the_arc_service_layer() -> None:
    ...
    assert not violations, (
        "ARC mutations must live in registry/arc/service/ — every atomicity, "
        "single-use, and encryption guarantee in this subsystem depends on one "
        "service owning the write:\n" + "\n".join(violations)
    )
...
def test_no_orm_write_outside_the_arc_service_layer() -> None:
    ...
    assert not violations, (
        "ARC ORM writes must live in registry/arc/service/:\n" + "\n".join(violations)
    )
