...
`retrieve_claims` tool that did not exist, and a conformance test pinned that promise
...
def test_the_claim_surface_offers_all_three_reads() -> None:
    ...
    id; `retrieve_claims` takes prose. Without the third, an agent that does not already
    know what to ask for has no way in -- which is the case the semantic index was built
    for.
    """
    registered = _tools()
    for name in ("query_claims", "get_claim", "retrieve_claims"):
        assert name in registered, f"the agent surface is missing {name}"


def test_the_documented_tool_actually_exists() -> None:
    """The reference named `retrieve_claims` before anything implemented it.
    ...
    assert "## retrieve_claims" in body, "the tool exists but the reference does not describe it"
    assert "retrieve_claims" in _tools()
