...
async def test_a_ninety_day_aggregate_read_stays_under_the_latency_budget(pg_container: str) -> None:
    ...
    try:
        ...
        assert rest.distinct_actors is not None and rest.distinct_actors > 0, (
        ...
        assert p95 <= _MAX_READ_P95_MS, (
            f"a {_WINDOW_DAYS}-day aggregate read takes {p95:.2f}ms at p95, "
            f"over the {_MAX_READ_P95_MS}ms budget"
        )
    finally:
    ...
    try:
        ...
        async with factory() as session:
            written = int(
                (
                    await session.execute(
                        text("SELECT count(*) FROM usage_events WHERE tenant_id = :t"), {"t": tenant}
                    )
                ).scalar_one()
            )
