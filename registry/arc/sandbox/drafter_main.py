...
Run via ``python -m registry.arc.sandbox.drafter_main`` -- one process per
...
parser's (``registry.arc.sandbox.parser_main``): the two are separate
...
``registry.wiring``, or ``registry.config`` -- no database credential, no
...
- **Parsed envelope.** Arrives over the one authenticated request, not via
  ...
  ``registry.arc.schemas.parser_output.ParsedSourceEnvelope``), produced by
...
- **No model artifact.** ``ARC_DRAFTER_MODEL_ARTIFACT_PATH`` is never read
  ...
  one, documented in full in ``registry.arc.schemas.drafter_output``'s own
...
from registry.arc.schemas.drafter_output import MAX_DECLINED_FIELD_PATHS, DrafterRefusal, DrafterSuccess
...
def draft_from_envelope(
    ...
    *,
    ...
    See this module's own docstring and `registry.arc.schemas.
