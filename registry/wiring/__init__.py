...
`registry.main.create_app` is the only place these modules get assembled
...
per-request container accessor) at the top level: `registry.wiring.services`
names the service-*construction* module below, not that function, so
`from registry.wiring import services` has to resolve to the module. Import
the accessor from where it lives instead: `from registry.wiring.container
import services`.
"""
