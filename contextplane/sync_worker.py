...
Run via ``python -m contextplane.sync_worker``.
...
(``contextplane.storage.pg`` for the engine/session factory,
``contextplane.wiring.services.build_core_services`` for the ``CatalogService``
...
Usage::
    ...
    python -m contextplane.sync_worker
...
async def _run() -> None:
    ...
    # `python -m contextplane.sync_worker --help`-shaped invocations (none exist
    # today, but nothing stops a future one) don't pay for the whole service
    # graph just to parse arguments.
    from contextplane.config import get_settings  # noqa: PLC0415 - worker entrypoint
    from contextplane.ingest.runner import create_scheduler, register_sync_jobs  # noqa: PLC0415 - worker entrypoint
    from contextplane.service.memory.claim_writer import ClaimService  # noqa: PLC0415 - worker entrypoint
    from contextplane.service.memory.source_governance import (  # noqa: PLC0415 - worker entrypoint
        SourceGovernanceService,
    )
    from contextplane.service.memory.source_ingest import SourceIngestService  # noqa: PLC0415 - worker entrypoint
    from contextplane.storage.pg import create_engine, get_session_factory  # noqa: PLC0415 - worker entrypoint
    from contextplane.wiring.services import build_core_services  # noqa: PLC0415 - worker entrypoint
    ...
    # the same way `contextplane/wiring/jobs.py::start` builds its own copy for
    ...
    # uses in `contextplane/wiring/jobs.py::start`.
    ...
    try:
        ...
        # back — mirrors the API process's own shutdown in `contextplane/main.py`.
