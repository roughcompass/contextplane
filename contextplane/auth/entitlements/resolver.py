...
from contextplane.auth.entitlements import parser as entitlement_parser
from contextplane.auth.entitlements.actor_store import (
    DisabledTenantError,
    upsert_entitlement_actor,
    upsert_entitlement_tenant,
)
from contextplane.auth.resolver import (
    AuditIdentity,
    ClaimResolverBase,
    ResolvedIdentity,
    TenantGrant,
...
class EntitlementResolver(ClaimResolverBase):
    ...
    async def _emit_stale_cache_event(
        ...
        except Exception:  # noqa: BLE001 - best-effort audit write, see docstring above
            _log.exception(
