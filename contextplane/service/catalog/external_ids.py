...
* Uniqueness is ``(tenant_id, external_system_slug, external_id)``.  A duplicate
  insert raises :class:`~contextplane.exceptions.ConflictError` with the existing PK
...
class ExternalIdService:
    ...
    async def register_external_system(
        ...
        :class:`~contextplane.exceptions.ConflictError`.
    ...
    async def delete_external_system(self, ctx: TenantContext, slug: str) -> None:
        ...
        Raises :class:`~contextplane.exceptions.NotFoundError` if the slug does not
    ...
    async def delete_external_id(
        ...
        Raises :class:`~contextplane.exceptions.NotFoundError` if the row does not
    ...
    async def update_external_id(
        ...
        and ``entity_id``.  Raises :class:`~contextplane.exceptions.NotFoundError`
        ...
        Returns the updated :class:`~contextplane.types.ExternalIdRef`.
        """
        now = self._clock.now()
        async with self._session_factory() as session, session.begin():
