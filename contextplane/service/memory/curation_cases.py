...
#: What a *person* may record. Equal to `DISPOSITIONS` today, and deliberately a
#: separate name anyway.
#:
#: A withdrawn attempt at a policy-written disposition established that the two
#: transports do not agree about their accepted set on their own: the HTTP route
#: hand-writes a `Literal` and the MCP tool takes a bare `str`, so a value that
#: was meant to be service-only was reachable from the agent-facing surface while
#: a conformance test inspecting only the HTTP model reported them closed. Both
#: transports now build from this name, so the next policy-only disposition
#: cannot reopen that gap.
#:
#: See `OPERATOR_DISPOSITIONS` below.
...
#: The dispositions no transport offers, because a service would write them.
#: Empty today. Kept as a name so that adding one is a change to this line --
#: which both transports and a conformance test already read -- rather than a
#: change somebody has to remember to make in three places.
POLICY_ONLY_DISPOSITIONS: Final[frozenset[str]] = frozenset()
