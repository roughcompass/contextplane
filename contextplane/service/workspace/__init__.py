...
``contextplane.service.workspace`` — that import path is this package's public
contract. A caller wanting ``WorkspaceRef``, ``WorkspaceNotFound``,
``WorkspaceEntryRef``, ``SearchResult``, or ``PurgeResult`` imports the
concern module that owns it directly (``contextplane.service.workspace.core``,
``.entries``, ``.search``, ``.purge``) — the same way a catalog caller
imports ``contextplane.service.catalog.entity`` directly. Nothing below the
...
from contextplane.service.workspace._shared import AuditWriter
from contextplane.service.workspace.core import _CoreMethods
from contextplane.service.workspace.entries import _EntryMethods
from contextplane.service.workspace.purge import _PurgeMethods
from contextplane.service.workspace.search import _SearchMethods
