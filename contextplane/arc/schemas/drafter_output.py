...
Sibling of `contextplane.arc.schemas.parser_output`, same reasoning: the API
...
(`contextplane.arc.sandbox.ipc`) agree on one closed shape defined once, here,
imported by both the sandboxed process (`contextplane.arc.sandbox.drafter_main`)
and the API-side caller (`contextplane.arc.service.drafter`). This module is
...
That wire type lives in `contextplane.api.schemas.arc_authoring_profiles`, which
imports pydantic models built from `contextplane.arc.schemas.authoring_profile_shapes`
...
draws. `contextplane.arc.service.drafter` (the API-side adapter) is what turns
...
model artifact exists (`contextplane/arc/drafter/model_decision.json` records
...
class DraftedCitation(_ClosedModel):
    ...
    (`contextplane.api.schemas.arc_authoring_shared.Citation`) -- deliberately
