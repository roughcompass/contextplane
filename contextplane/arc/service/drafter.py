...
committed decision (`contextplane/arc/drafter/model_decision.json`, `outcome:
...
`contextplane.arc.sandbox.ipc` unchanged for both.** The API process never
...
existing parser sandbox (`contextplane.arc.sandbox.parser_main`, already built
...
(`contextplane.arc.sandbox.drafter_main`). Each sandbox gets its own socket
...
from contextplane.arc.schemas import drafter_output
...
from contextplane.arc.service.queries import drafter as queries
...
from contextplane.types import Clock, JSONValue
...
async def _run_sandbox_pipeline(
    ...
    (see e.g. `contextplane/service/retrieval/search.py`)."""
