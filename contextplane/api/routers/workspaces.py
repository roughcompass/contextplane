...
- NotFoundError (contextplane.exceptions; entry-row-missing, distinct from the
  perceivability-conflated WorkspaceNotFound above) → HTTP 404.
- ValidationError (contextplane.exceptions) → HTTP 422, message passed through.
...
def _ws_exc_to_http(exc: Exception) -> HTTPException:
    ...
    directly: they predate this task and aren't part of the contextplane.exceptions
    ...
    map_catalog_error, the one table every router maps contextplane.exceptions
