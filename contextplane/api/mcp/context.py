...
``contextplane.api.mcp.tools`` reads that state through this module rather than
importing the ContextVars by name, so a test that patches
``contextplane.api.mcp.context._resolve_tenant`` (or any other name here) reaches
...
async def _resolve_tenant(
    ...
    # REST middleware (`contextplane.api.middleware.tenant`) reads the same
    ...
    # patch_validator_for_actor patches contextplane.api.auth.oidc's own
    ...
    from contextplane.api.auth.oidc import validate_oidc_token  # noqa: PLC0415 - test patch target, see comment above
