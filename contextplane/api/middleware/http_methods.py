...
DELETE (``contextplane.api.routers.memory``) soft-invalidates on a different
...
# ``contextplane.config.Settings.http_methods_mode`` /
...
def get_mode_settings() -> tuple[HttpMethodsMode, AliasSeparator]:
    ...
    The canonical defaults live in :class:`contextplane.config.Settings`
