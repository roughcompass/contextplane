...
if TYPE_CHECKING:  # pragma: no cover - typing only
    ...
    from contextplane.extraction.response_factory import PanelMember
...
class PanelOutcome:
    ...
    **Not a blended number.** A 2-1 panel records that it was 2-1, because a
    criterion three judges disagree about is the one most worth a human's time.
    Averaging their confidences into a single figure would destroy exactly the
    signal the panel costs 3x to produce.

    Computed rather than stored: it is a view over rows that already exist, and a
    stored copy would be a second answer that could not be corrected when a
    member was re-judged.
    """
    ...
    def votes(self) -> dict[str, int]:
        """How the panel split, by verdict."""
        tally: dict[str, int] = {}
    ...
    def majority(self) -> str | None:
        """The verdict more than half the panel reached, or `None` on a tie.

        `None` rather than a tie-break, and that is the decision rather than an
        omission: a panel that split evenly has not decided, and inventing a
        winner would report agreement nobody reached. An even panel is a
        configuration mistake this makes visible instead of papering over.
        """
        tally = self.votes
        if not tally:
            return None
        best = max(tally.values())
        leaders = [verdict for verdict, count in tally.items() if count == best]
        if len(leaders) != 1 or best * 2 <= len(self.judgements):
            return None
        return leaders[0]

    @property
    def is_split(self) -> bool:
        """Whether the panel disagreed at all. The interesting output."""
...
class JudgementService:
    ...
    def __init__(
        ...
        *,
        ...
        calibration: JudgeCalibrationService | None = None,
    ) -> None:
    ...
    def panel_size(self) -> int:
        """How many judges this deployment has configured.

        A panel of one is what a deployment with a single judge already has; the
        opt-in is asking for a panel at all, not configuring a different feature.
        """
    ...
    async def _judge_with(
        ...
        *,
        ...
        """One judge, one answer, both criteria. The path every member takes."""
        ...
        request = JudgementRequest(
            ...
            model_id=resolved_model(selector=selector, pinned=model_pin),
    ...
    async def judge_panel(self, ctx: TenantContext, *, simulation_id: uuid.UUID) -> tuple[PanelOutcome, ...]:
        """Run every configured judge over one answer, and leave the split visible.

        Opt-in, and 3x the cost of a single judge — right for a run that is
        gating a launch decision, wrong for the interactive loop where nobody is
        making one. That is why this is a separate operation rather than a
        default.

        Family diversity is required *across the panel*, not merely against the
        candidate: three judges from one family cancel nothing.

        Sequential rather than concurrent. Three concurrent provider calls would
        contend for the same event loop and the same rate limits, and each
        member's `duration_ms` would become a fact about the batch rather than
        about the judge.
        """
        if not self._panel:
            msg = (
                "no panel is configured: a panel needs at least one judge. Set JUDGE_PROVIDER, and "
                "JUDGE_2_PROVIDER and JUDGE_3_PROVIDER for the three-family panel a launch gate "
                "wants. Each must be a different family."
            )
            raise SimulationUnavailable(msg)
        assert_panel_families_differ(self._panel)
        for member in self._panel:
            assert_families_differ(
                candidate_model=resolved_model(selector=self._candidate_selector, pinned=self._candidate_model_pin),
                candidate_provider=self._candidate_selector,
                judge_model=resolved_model(selector=member.selector, pinned=member.model_pin),
                judge_provider=member.selector,
            )

        for member in self._panel:
            await self._judge_with(
                ctx,
                model_pin=member.model_pin,
                panel_position=member.position,
                provider=member.provider,
                selector=member.selector,
                simulation_id=simulation_id,
            )
        return self.panel_outcomes(await self.judgements_of(ctx, simulation_id))

    @staticmethod
    def panel_outcomes(judgements: tuple[Judgement, ...]) -> tuple[PanelOutcome, ...]:
        """Group judged rows by criterion, in the rubric's own order.

        A static method rather than a free function because it is the one place
        the grouping rule lives, and a second caller grouping differently would
        report a different split from the same rows.
        """
        by_criterion: dict[str, list[Judgement]] = {}
        for entry in judgements:
            by_criterion.setdefault(entry.criterion, []).append(entry)
        return tuple(
            PanelOutcome(criterion=name, judgements=tuple(by_criterion[name]))
            for name in JUDGED_CRITERIA
            if name in by_criterion
        )

    async def calibrated_tuples(self) -> frozenset[tuple[str, str, str]]:
        """Which pinned tuples have a fit selected, for the surface to render.

        Empty when no calibration service is wired, which is the safe direction:
        a surface reading an empty set renders every verdict as unproven, and an
        unproven verdict shown as unproven is the honest failure.
        """
        if self._calibration is None:
            return frozenset()
        return await self._calibration.calibrated_tuples()
