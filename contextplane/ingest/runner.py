...
* One ``AsyncIOScheduler`` is shared between the embedding-drain job and all
  sync-source jobs.  ``create_scheduler`` is called by
  ``contextplane/wiring/jobs.py`` (the API process's scheduler) and
  ``contextplane/sync_worker.py`` (the standalone sync-worker process); callers
  must start / shutdown it.
...
from contextplane.ingest.connector_registry import UnknownConnectorError, get_connector
from contextplane.metrics import observe_sync_run
from contextplane.service.catalog.core import CatalogService
from contextplane.service.memory.claim_authority import EVIDENCE_CONNECTOR_RUN, Evidence
from contextplane.service.memory.source_ingest import Candidate, SourceIngestService
from contextplane.storage.models import Actor, SyncRun, SyncSource, WebhookDelivery
...
# ---------------------------------------------------------------------------
# Scheduler factory (called by contextplane/wiring/jobs.py and contextplane/sync_worker.py)
# ---------------------------------------------------------------------------
