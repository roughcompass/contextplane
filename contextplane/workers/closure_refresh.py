...
- This module is the consumer only. The producer -- `enqueue_closure_refresh`,
  which writes the outbox row inside the edge-write transaction -- lives in
  `service/retrieval/closure_index.py`, beside the read path it keeps warm,
  so that writing one outbox row does not make the service layer import a
  worker.
"""
...
from contextplane.metrics import observe_queue_depth, observe_worker_run
from contextplane.service.retrieval import RetrievalService
from contextplane.types import Clock, SystemClock, TemporalFilter
