...
2. `attach_core_services` / `_wire_arc` / `wire_auth_context` — once `app`
   ...
   (`UsageWriter`, `ArcServices`, `AuthContext`) for `contextplane.main.create_app`
   to thread straight into stage 3. A field is also attached to `app.state`
   here only when something outside `contextplane.wiring` still reads it live —
...
This module is the reason `contextplane.wiring` exists: before the split, all
...
class ArcServices:
    ...
    and memory/claims service, attaches the handful `contextplane.api.routers.arc`
...
def attach_core_services(
    ...
    `contextplane.main.create_app` threads it straight into
    ...
    """
    # Surviving bare readers: contextplane.api.middleware.tenant (settings) and
    # contextplane.api.mcp.context._resolve_tenant (settings) both read this live
    # rather than through the container — see either one's own comment for why.
    app.state.settings = settings
    # Surviving bare readers: contextplane.api.middleware.idempotency.get_idempotency_context
    ...
    # Surviving bare reader: contextplane.usage.recording._writer reads this live —
    ...
    # Surviving bare readers: contextplane.api.routers.admin_lifecycle and
    ...
    # Surviving bare readers: contextplane.api.routers._common, retrieval,
    # admin_sync, contextplane.ingest.webhook -- none has migrated to the typed
    # container yet.
    app.state.catalog = core.catalog
    # Surviving bare readers: contextplane.api.routers.retrieval, graph.
    app.state.retrieval = core.retrieval
    # Surviving bare reader: contextplane.api.routers.external_ids.
    app.state.external_ids = core.external_ids
    # Surviving bare readers: contextplane.ingest.webhook, contextplane.api.routers.admin_sync.
    app.state.scheduler = scheduler
    # Surviving bare reader: contextplane.api.routers.capabilities.
    app.state.visibility = core.visibility
    # Surviving bare reader: contextplane.api.routers.adoptions.
    app.state.adoption = core.adoption
    # Surviving bare reader: contextplane.api.routers.graph.
    app.state.projections = core.projections
    # Surviving bare reader: contextplane.api.routers.subscriptions.
    app.state.subscriptions = core.subscriptions
    # Surviving bare reader: contextplane.api.routers.notifications.
    app.state.notifications = core.notifications
    # Surviving bare reader: contextplane.api.routers.breaking_change.
    app.state.breaking_change = core.breaking_change
    # Surviving bare reader: contextplane.api.routers.integrations.
    app.state.integrations = core.integrations
    # Surviving bare reader: contextplane.api.routers.interface.
...
def _wire_arc(
    ...
    *,
    ...
    # Surviving bare reader: contextplane.api.routers.arc.
...
def wire_auth_context(
    ...
    """
    oidc_cache = _OidcCache()
    # Surviving bare readers: contextplane.api.middleware.tenant (oidc_cache) and
    # contextplane.api.mcp.context._resolve_tenant (oidc_cache) both read this live
    # rather than through the container — see either one's own comment for why.
    ...
    # contextplane.api.middleware.tenant and contextplane.api.mcp.context._resolve_tenant
    ...
    # reader is contextplane.main's own lifespan, which closes it on shutdown
...
def build_services_container(
    *,
    ...
    `wire_auth_context`, and `contextplane.wiring.routes.register` (the
    workspace singleton and the erasure registry, both built after the
    router table is mounted) handed back. `contextplane.main.create_app` is the
