...
ALLOWLIST: tuple[tuple[str, str, str], ...] = (
    (
        ...
        "accept-path call to _validate_semver_attribute relies on a code comment ('no raise').",
    ),
    (
        "tests/unit/test_semver_enforcement.py",
        "test_missing_version_is_noop",
        "same shape, missing-version-key branch.",
    ),
    (
        "tests/unit/test_semver_enforcement.py",
        "test_empty_attributes_is_noop",
        "same shape, empty-attributes branch.",
    ),
    (
        "tests/unit/test_semver_enforcement.py",
        "test_none_version_is_noop",
        "same shape, explicit version=None branch.",
    ),
    (
        "tests/unit/test_usage_recording.py",
        "test_stashing_never_raises",
        "calls stash_request_identity and checks nothing beyond the function name's own claim.",
    ),
    (
        "tests/unit/test_usage_recording.py",
        "test_stashing_a_result_count_never_raises",
        "same shape as test_stashing_never_raises, the result-count stash.",
    ),
    (
        "tests/unit/test_usage_recording.py",
        "test_recording_with_no_writer_configured_is_silent",
        "calls record_rest_usage with no writer configured and checks nothing beyond a code comment.",
    ),
    (
        "tests/unit/test_usage_recording.py",
        "test_a_writer_that_explodes_never_reaches_the_request",
        "calls record_rest_usage with a writer that raises and never checks that the exception was actually "
        "contained -- only a bare call, no assertion either way.",
    ),
    (
        "tests/unit/test_visibility_service.py",
        "test_validate_visibility_private_ok",
        "accept-path call to _validate_visibility_input relies on a code comment ('no raise').",
    ),
    (
        "tests/unit/test_visibility_service.py",
        "test_validate_visibility_public_ok",
        "same shape, public visibility branch.",
    ),
    (
        "tests/unit/test_visibility_service.py",
        "test_validate_visibility_tenant_shared_with_acl_ok",
        "same shape, tenant-shared-with-acl branch.",
    ),
    (
    ...
    (
        ...
        "test_exception_from_run_once_is_swallowed_not_raised",
        "registers a failing run_once and never checks that the failure was actually logged/swallowed -- the "
        "name promises the property, the body does not check it.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_write_entries_producer_own_actor_ws_active",
        "allow-branch call to _assert_can_write_entries; the guard's job is to raise on denial, and the allow "
        "branches never check anything beyond the absence of that raise.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_write_entries_admin_tenant_ws_active",
        "same shape as test_write_entries_producer_own_actor_ws_active, admin-on-tenant-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_write_entries_admin_producer_own_actor_ws",
        "same shape, admin+producer dual-role branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_update_ws_producer_own_actor_ws_active",
        "same shape against _assert_can_update_workspace, producer-on-own-actor-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_update_ws_admin_tenant_ws_active",
        "same shape, admin-on-tenant-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_update_ws_admin_producer_own_actor_ws",
        "same shape, admin+producer dual-role branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_delete_ws_producer_own_actor_ws_active",
        "same shape against _assert_can_delete_workspace, producer-on-own-active-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_delete_ws_producer_own_actor_ws_archived",
        "same shape, producer-on-own-archived-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_delete_ws_admin_tenant_ws_active",
        "same shape, admin-on-active-tenant-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_delete_ws_admin_tenant_ws_archived",
        "same shape, admin-on-archived-tenant-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_delete_ws_admin_producer_own_actor_ws",
        "same shape, admin+producer dual-role branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_archive_ws_producer_own_actor_ws",
        "same shape against _assert_can_archive_workspace, producer-on-own-actor-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
        "test_archive_ws_admin_tenant_ws",
        "same shape, admin-on-tenant-workspace branch.",
    ),
    (
        "tests/unit/test_workspace_rbac_predicates.py",
