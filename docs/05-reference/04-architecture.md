...
- **Audit log** (`audit_log` table) is monthly-partitioned. The `check_audit_partition_ages` startup hook and recurring scheduler job warn when a partition is older than `audit_partition_max_age_days`. Archiving a single old partition is a manual operator procedure (`DETACH PARTITION CONCURRENTLY`, dump, then drop) — see [Audit log partition archival](../06-operations/01-ops.md#audit-log-partition-archival). Rebuilding the whole table (repartitioning) is `scripts/partition_migrate.py`, which derives its shadow table from the live one at run time, copies in committed month chunks, then swaps under a bounded `ACCESS EXCLUSIVE` lock.
