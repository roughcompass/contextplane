...
the Context Plane service exposes a REST API under `/v1` and an MCP surface at `/mcp`. This document explains how to discover the live schema, covers the major resource groups and their semantics, and describes common headers and error shapes.
...
A registry of upstream systems whose IDs are mapped onto entities via the external-ID endpoints.
...
|---|---|---|
| `rest` (default) | `PATCH` / `PUT` / `DELETE` | `PATCH /v1/capabilities/{id}` |
